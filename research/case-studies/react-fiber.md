...
Source: Meta, [React 16: A look inside an API-compatible rewrite of our
...
public API kept essentially stable. The target enabled capabilities that were
difficult in the former implementation, including asynchronous rendering.
...
| --- | --- |
| Construction mode | Complete internal architecture replacement |
| Transition mode | Incremental coexistence, dogfooding, and rollout |
| Compatibility baseline | Public React API and behavior-oriented tests |
| Asset salvage | Public tests and external component ecosystem |
...
The case shows why construction and transition modes must be separate. It also
shows an asset-level test disposition: the suite was valuable, but tests that
made old implementation details authoritative needed change.
